...
$(function () {
    $('.main_content .tap_menu button').on('click', function () {
        let idx = $(this).index(); // 클릭한 것의 번호를 가져와서 저장하기
        console.log(idx)
        $('.main_content .tap_menu button').removeClass('on');
        $(this).addClass('on');


        $('.main_content .tap_content .con').removeClass('on');
        $('.main_content .tap_content .con').eq(idx).addClass('on');
    });
})
